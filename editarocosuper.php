...
if (!isset($_GET['id'])) {
    header("Location: listarocosuper.php?erro=sem_id");
    exit();
}
...
if (!$oc) {
    header("Location: listarocosuper.php?erro=nao_existe");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($success) {
        ...
        mysqli_query($link, "INSERT INTO logs (descricao, datahora, IDutl)
                             VALUES ('Ocorrência editada pelo SUPERADMIN (ID $IDoc)', '$fdatahora', '$IDutl')");

        header("Location: listarocosuper.php?sucesso=editado");
        exit();
    } else {
