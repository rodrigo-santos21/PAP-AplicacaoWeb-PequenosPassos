...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$permitido) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            header("Location: listarocoedu.php?sucesso=adicionada");
            exit();
        } else {
            header("Location: adicionaroco.php?erro=processar");
            exit();
        }
    }
}
                ...
                <form method="post" class="space-y-5">
                    ...
                    <script>
                        document.getElementById('tipoSelect').addEventListener('change', function() {
                            document.getElementById('outroCampo').style.display =
...
<!-- TOAST: erro ao registar -->
<?php if (isset($_GET['erro']) && $_GET['erro'] === 'processar'): ?>
<script>
window.addEventListener("load", () => {
    mostrarMensagem("reset", "Erro ao registar a ocorrência.");
});
</script>
<?php endif; ?>
...
<script>
window.addEventListener("load", () => {
    mostrarMensagem("reset", "Não tem permissão para registar ocorrência nesta criança.");
});
</script>
<?php endif; ?>
