...
/* 🔍 Pesquisa (descrição + tipo_outro) */
if (!empty($pesquisa)) {
    $p = mysqli_real_escape_string($link, $pesquisa);
    $where .= " AND (descricao LIKE '%$p%' OR tipo_outro LIKE '%$p%')";
}
...
/* 🔤 Ordenação */
$ordemSQL = "ORDER BY IDoc DESC"; // padrão
...
<body class="bg-gray-100 dark:bg-gray-900 dark:text-gray-100 min-h-screen">
    ...
    <div class="flex min-h-screen flex-col lg:flex-row">
        ...
        <main class="flex-1 p-6 lg:p-10 lg:ml-[20%] overflow-y-auto">
            ...
            <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-8">Ocorrências da Creche</h1>
            ...
            <form method="GET" id="filtrosForm"
                ...
                <div>
                    ...
                    <select name="educador"
                        ...
                        $resEdu = mysqli_query($link, "
                            SELECT DISTINCT IDedu 
                            FROM ocorrencia 
                            WHERE estado = 1 AND IDedu IS NOT NULL
                        ");

                        while ($e = mysqli_fetch_assoc($resEdu)):
                            $idEdu = $e['IDedu'];

                            $resNome = mysqli_query($link, "SELECT IDutl FROM educador WHERE IDedu = $idEdu");
                            if ($resNome && mysqli_num_rows($resNome) > 0) {
                                $idU = mysqli_fetch_assoc($resNome)['IDutl'];

                                $resU = mysqli_query($link, "SELECT nome FROM utilizador WHERE IDutl = $idU");
                                $nomeEdu = mysqli_num_rows($resU) ? mysqli_fetch_assoc($resU)['nome'] : "Educador #$idEdu";
                            } else {
                                $nomeEdu = "Educador #$idEdu";
                            }
                        ?>
                            <option value="<?= $idEdu ?>"
                ...
                <div class="flex mt-6 items-center justify-end">
                    <button type="button"
                        onclick="window.location.href='listarocosuper.php'"
            ...
            <div class="w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8">
                ...
                <?php else: ?>
                    ...
                    <div class="bg-green-50 dark:bg-green-900/30 shadow-md rounded-lg p-6 hover:shadow-xl transition">
                        ...
                        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-2"><?= $criNome ?></h2>

                        <p class="text-gray-700 dark:text-gray-300"><strong>ID:</strong> <?= $o['IDoc'] ?></p>
                        <p class="text-gray-700 dark:text-gray-300"><strong>Data:</strong> <?= $o['datahora'] ?></p>
                        <p class="text-gray-700 dark:text-gray-300"><strong>Tipo:</strong> <?= $tipoFinal ?></p>
                        <p class="text-gray-700 dark:text-gray-300"><strong>Gravidade:</strong> <?= $o['gravidade'] ?></p>
                        <p class="text-gray-700 dark:text-gray-300"><strong>Criado por:</strong> <?= $eduNome ?></p>
...
<div id="modalEliminar" 
    ...
    <div class="bg-white dark:bg-gray-800 dark:text-gray-100 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Confirmar Desativação</h2>
...
<script>
    ...
    document.getElementById("btnConfirmarEliminar").addEventListener("click", function () {
        ...
        .then(res => {
            ...
            if (res === "ok") {
                fecharModal();
                mostrarMensagem("eliminar", "Ocorrência desativada com sucesso!");
                setTimeout(() => location.reload(), 1200);
            } else {
                mostrarMensagem("reset", "Erro ao desativar ocorrência.");
            }
        });
    });
</script>
