...
if (!isset($_GET['id'])) {
    header("Location: listarcrisuper.php?erro=sem_id");
    exit();
}
...
// Se não existir
if (!$crianca) {
    header("Location: listarcrisuper.php?erro=nao_existe");
    exit();
}
    ...
    if ($success) {
        ...
        if (empty($educadoresSelecionados)) {
            ...
            mysqli_query($link, "INSERT INTO logs (descricao, datahora, IDutl)
                                VALUES ('Superadmin editou criança (ID $id)', '$fdatahora', '$criadopor')");

            header("Location: listarcrisuper.php?sucesso=editado");
            exit();
        }
    } else {
