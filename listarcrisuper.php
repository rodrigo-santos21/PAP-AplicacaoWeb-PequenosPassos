...
/* 🚻 Sexo */
if (!empty($sexoFiltro)) {
    $sx = mysqli_real_escape_string($link, $sexoFiltro);
    $where .= " AND sexo = '$sx'";
}

/* 🔤 Ordenação */
$ordemSQL = "ORDER BY IDcri DESC"; // padrão
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_id'])) {
    ...
    mysqli_query($link, "UPDATE crianca SET estado = 0 WHERE IDcri = $id");
        ...
        <div class="w-full bg-white shadow-lg rounded-lg p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <?php else: ?>
                    ...
                    <?php while ($cri = mysqli_fetch_assoc($result)) {
...
<script>
    ...
    document.getElementById("btnConfirmarEliminar").addEventListener("click", function () {
        ...
        .then(res => {
            ...
            if (res === "ok") {
                fecharModal();
                mostrarMensagem("eliminar", "Criança eliminada com sucesso!");
                setTimeout(() => location.reload(), 1200);
            } else {
                mostrarMensagem("reset", "Erro ao eliminar criança.");
            }
        });
    });
